...
bool Intersection::Intersect(Body& a, Body& b, const float dt, Contact& contact)
{
		...
		&& b.shape->GetType() == Shape::ShapeType::SHAPE_SPHERE)
	{
		...
		if (Intersection::SphereSphereDynamic(*sphereA, *sphereB,
...
bool Intersection::RaySphere(const Vec3& rayStart, const Vec3& rayDir, const Vec3& sphereCenter, const float sphereRadius, float& t0, float& t1)
{
...
bool Intersection::SphereSphereDynamic(const ShapeSphere& shapeA, const ShapeSphere& shapeB,
